...
class SubClient(client.Client):
    ...
    async def get_invite_codes(self, status: str = "normal", start: int = 0, size: int = 25):
        async with self.session.get(
                ...
                headers=self.parse_headers()) as response:
    ...
    async def generate_invite_code(self, duration: int = 0, force: bool = True):
        ...
        async with self.session.post(f"{self.api}/g/s-x{self.comId}/community/invitation",
                                     headers=self.parse_headers(data=data), data=data) as response:
            if response.status != 200:
                return exceptions.CheckException(await response.text())
            else:
    ...
    async def post_blog(self, title: str, content: str, imageList: list = None, captionList: list = None,
        ...
        async with self.session.post(f"{self.api}/x{self.comId}/s/blog", headers=self.parse_headers(data=data),
    ...
    async def post_wiki(self, title: str, content: str, icon: str = None, imageList: list = None, keywords: str = None,
        ...
        async with self.session.post(f"{self.api}/x{self.comId}/s/item", headers=self.parse_headers(data=data),
                                     data=data) as response:
            if response.status != 200:
                return exceptions.CheckException(await response.text())
            else:
    ...
    async def edit_blog(self, blogId: str, title: str = None, content: str = None, imageList: list = None,
        ...
        async with self.session.post(f"{self.api}/x{self.comId}/s/blog/{blogId}", headers=self.parse_headers(data=data),
                                     data=data) as response:
            if response.status != 200:
                return exceptions.CheckException(await response.text())
            else:
    ...
    async def check_in(self, tz: int = -timezone // 1000):
        ...
        async with self.session.post(f"{self.api}/x{self.comId}/s/check-in", headers=self.parse_headers(data=data),
                                     data=data) as response:
            if response.status != 200:
                return exceptions.CheckException(await response.text())
            else:
    ...
    async def edit_profile(self, nickname: str = None, content: str = None, icon: BinaryIO = None,
                           ...
                           colors: list = None, defaultBubbleId: str = None):
        mediaList = []
    ...
    async def start_chat(self, userId: Union[str, list], message: str, title: str = None, content: str = None,
        ...
        async with self.session.post(f"{self.api}/x{self.comId}/s/chat/thread", headers=self.parse_headers(data=data),
                                     data=data) as response:
            if response.status != 200:
                return exceptions.CheckException(await response.text())
            else:
    ...
    async def flag(self, reason: str, flagType: int, userId: str = None, blogId: str = None, wikiId: str = None,
        ...
        **Returns**
        ...
        async with self.session.post(f"{self.api}/x{self.comId}/s/{flg}", headers=self.parse_headers(data=data),
                                     data=data) as response:
            if response.status != 200:
                return exceptions.CheckException(await response.text())
            else:
        ...
        if doNotDisturb is not None:
            if doNotDisturb:
                ...
                async with self.session.post(
                        ...
                        headers=self.parse_headers(data=data), data=data) as response:
                    if response.status != 200:
                        res.append(exceptions.CheckException(json.loads(await response.text())))
                    else:
    ...
    async def send_active_obj(self, tz: int = -timezone // 1000, timers: list = None):
        data = {"userActiveTimeChunkList": timers, "timestamp": int(timestamp() * 1000),
                "optInAdsFlags": 2147483647, "timezone": tz}
        ...
        **Returns**
        ...
        async with self.session.get(f"{self.api}/x{self.comId}/s/block?start={start}&size={size}",
            ...
            if response.status != 200:
                ...
                return json.loads(await response.text())["blockerUidList"]
    ...
    async def get_notices(self, start: int = 0, size: int = 25):
        ...
        async with self.session.get(
            ...
            if response.status != 200:
                ...
                return json.loads(await response.text())["noticeList"]
    ...
    async def warn(self, userId: str, reason: str = None):
        ...
        async with self.session.post(f"{self.api}/x{self.comId}/s/notice", headers=self.parse_headers(data=data),
                                     data=data) as response:
            if response.status != 200:
                return exceptions.CheckException(await response.text())
            else:
    ...
    async def create_wiki_category(self, title: str, parentCategoryId: str, content: str = None):
        ...
        async with self.session.post(f"{self.api}/x{self.comId}/s/item-category", headers=self.parse_headers(data=data),
                                     data=data) as response:
            if response.status != 200:
                return exceptions.CheckException(await response.text())
            else:
